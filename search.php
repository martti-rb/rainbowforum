...
// Define some vars
$limit_days		= array(0 => $user->lang['ALL_RESULTS'], 1 => $user->lang['1_DAY'], 7 => $user->lang['7_DAYS'], 14 => $user->lang['2_WEEKS'], 30 => $user->lang['1_MONTH'], 90 => $user->lang['3_MONTHS'], 180 => $user->lang['6_MONTHS'], 365 => $user->lang['1_YEAR'], 730 => $user->lang['2_YEARS']);
$sort_by_text	= array('a' => $user->lang['SORT_AUTHOR'], 'c' => $user->lang['SORT_COLOR'], 't' => $user->lang['SORT_TIME'], 'f' => $user->lang['SORT_FORUM'], 'i' => $user->lang['SORT_TOPIC_TITLE'], 's' => $user->lang['SORT_POST_SUBJECT']);
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	$sort_by_sql = array('a' => 'u.username_clean', 'c' => 't.topic_bg_color', 't' => (($show_results == 'posts') ? 'p.post_time' : 't.topic_last_post_time'), 'f' => 'f.forum_id', 'i' => 't.topic_title', 's' => (($show_results == 'posts') ? 'p.post_subject' : 't.topic_title'));
...
{
	$hide_filters_filter = request_var('filt', '');

	switch ($hide_filters_filter)
	{
		case 'en':
	...
	if (!$hide_filters)
	{	
		$u_reset_filters = append_sid($phpbb_root_path . 'search.' . $phpEx, 'st=0&sk=t&sd=d' . $u_show_results);
		$u_reset_filters .= ($search_id) ? '&amp;search_id=' . $search_id : '';
		$u_reset_filters .= ($u_hilit) ? '&amp;keywords=' . urlencode(htmlspecialchars_decode($keywords)) : '';
		$u_reset_filters .= ($search_terms != 'all') ? '&amp;terms=' . $search_terms : '';
		$u_reset_filters .= ($topic_id) ? '&amp;t=' . $topic_id : '';
		$u_reset_filters .= ($author) ? '&amp;author=' . urlencode(htmlspecialchars_decode($author)) : '';
		$u_reset_filters .= ($author_id) ? '&amp;author_id=' . $author_id : '';
		$u_reset_filters .= ($u_search_forum) ? '&amp;fid%5B%5D=' . $u_search_forum : '';
		$u_reset_filters .= (!$search_child) ? '&amp;sc=0' : '';
		$u_reset_filters .= ($search_fields != 'all') ? '&amp;sf=' . $search_fields : '';
		$u_reset_filters .= ($return_chars != 300) ? '&amp;ch=' . $return_chars : '';
	}	
}
...
$template->assign_vars(array(
	...
	'S_HIDE_FILTERS_ACTION'		=> $u_search . $u_hide_filters,
	'S_RESET_FILTERS_ACTION'	=> $u_reset_filters,
	));	
...
// filters mod end	
	...
	if ($sql_where)
	{
		if ($show_results == 'posts')
		{
			...
			$sql = 'SELECT p.*, f.forum_id, f.forum_name, f.forum_parents, f.display_parent_forum, t.*, u.username, u.username_clean, u.user_sig, u.user_sig_bbcode_uid, u.user_colour
				FROM ' . POSTS_TABLE . ' p
					LEFT JOIN ' . TOPICS_TABLE . ' t ON (p.topic_id = t.topic_id)
					LEFT JOIN ' . FORUMS_TABLE . ' f ON (p.forum_id = f.forum_id)
					LEFT JOIN ' . USERS_TABLE . " u ON (p.poster_id = u.user_id)
				WHERE $sql_where"; // Display Parent Forum Name Mod
		}
		else
		{
			...
			$sql_select = 't.*, f.forum_id, f.forum_name, f.forum_parents, f.display_parent_forum'; // Display Parent Forum Mod
...
// Display Parent Forum Name Mod - Begin
				...
				if (!isset($forum_parent_names[$row['forum_id']]) && $row['display_parent_forum'])
				{
					$forum_parent_names[$row['forum_id']] = obtain_parent_forum_name($row);	
				}		
				$rowset[$row['topic_id']]['forum_name'] = $forum_parent_names[$row['forum_id']] . $row['forum_name'];
...
// Display Parent Forum Name Mod - Begin
				...
				$row['forum_name'] = $forum_parent_names[$row['forum_id']] . $row['forum_name'];
				
// Display Parent Forum Name Mod - End

// Calendar Mod - Begin
				if(!isset($topic_is_set[$row['topic_id']]))
				{
					$topic_list[] = $row['topic_id'];
					$topic_is_set[$row['topic_id']] = true;			
				}
// Calendar Mod - End			
...
// location mod begin
		...
		$topic_locations = new topic_locations($topic_list);
...
// link mod begin
		...
		$topic_link = new topic_link();	

// link mod end

// Topic Title Prefix Mod begin

		$prefix_data = obtain_topic_title_prefixes($topic_list);
...
// Calendar Mod - Begin	
				...
				'DATE_SUFFIX'			=> (isset($calendar_data[$result_topic_id]['suffix'])) ? $calendar_data[$result_topic_id]['suffix'] : '',   
				'U_CALENDAR_TOPIC' 		=> (isset($calendar_data[$result_topic_id]['start_jd'])) ? append_sid($phpbb_root_path . 'calendar.' . $phpEx, 'start_jd=' . ($calendar_data[$result_topic_id]['start_jd'] - CALENDAR_DAYS_OFFSET) . '&amp;t=' . $result_topic_id) : '',				

// Calendar Mod - End

// locations mod begin

				'LOCATION_TAG_STRING'	=> 	$topic_locations->get_tag_string($result_topic_id),
...
// Topic Title Prefix Mod - Begin
				...
				'TITLE_PREFIX'			=> (isset($prefix_data[$result_topic_id])) ? $prefix_data[$result_topic_id] : '',
