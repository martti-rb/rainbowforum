...
if ($links_forum)
{
	...
	$link_view = request_var('link_view', '');
	...
	{		
		$sql = 'UPDATE ' . USERS_TABLE . '
					SET user_link_view = ' . $user->link_view . '
					WHERE user_id = ' . $user->data['user_id'];
		$db->sql_query($sql);
	}		

	
	$u_link_view = '&link_view=';
	$u_link_view .= ($user->link_view) ? 'n' : 'y';
}


$topic_link = new topic_link($links_forum);

// link mod end

// archive mod begin

$archive_forum = ($config['archive_id'] == $forum_id) ? true : false;

// archive mod end

// filters mod begin

$sql_filters = ' AND t.topic_type IN (' . POST_NORMAL . ', ' . POST_STICKY . ')';



$topic_filters = new topic_filters($repl->lang('ALL_TOPICS'));
$topic_filters->init_forum_mode($forum_id, $sql_filters);
$archive_forum_list = $topic_filters->add_archive();
$prefix_names = ($topic_filters->add_prefix($forum_data['prefix_group_id']));
$topic_filters->add_calendar();
$topic_locations = new topic_locations();
$topic_filters->add_location($topic_locations);
$topic_filters->add_image_thumbnail();
$topic_filters->add_link_status();
$topic_filters->add_link_type($topic_link->type);
$topic_filters->add_link_source($topic_link->source);
$topic_filters->add_link_forum();



/*
$topic_filters->add_circle();
*/

$topic_filters->gen_sql();
$topic_filters->gen_u();
...
// $limit_days = array(0 => $repl->lang('ALL_TOPICS'), 1 => $user->lang['1_DAY'], 7 => $user->lang['7_DAYS'], 14 => $user->lang['2_WEEKS'], 30 => $user->lang['1_MONTH'], 90 => $user->lang['3_MONTHS'], 180 => $user->lang['6_MONTHS'], 365 => $user->lang['1_YEAR'], 730 => $user->lang['2_YEARS']);

// filter mod begin

$sql_approved = ($auth->acl_get('m_approve', $forum_id)) ? '' : ' AND t.topic_approved = 1';
$limit_days = $topic_filters->gen_limit_days($sql_approved);

// filter mod end

$sort_by_text = array(
	'a' => $user->lang['AUTHOR'], 
	'c' => $user->lang['COLOR'],		// topic color mod insert
...
$sort_by_sql = array(
	...
	'c'	=> 't.topic_bg_color', 			// topic color mod insert
	't' => 't.topic_last_post_time',
		// 'r'
	's' => 't.topic_title', 
	'v' => 't.topic_views');

// link mod // circle mod - modified	
if (!$links_circles_forum)
{
	$sort_by_text['r'] = $user->lang['REPLIES'];
	ksort($sort_by_text);
	$sort_by_sql['r'] = 't.topic_replies';
	ksort($sort_by_sql);
}	
...
// archive mod begin // topic title prefix mod begin
...
$topic_filters->set_sql_suf($sql_limit_time . $sql_approved);

if ($config['show_topic_filters'])   // quick fix when overloaded
{
	$topic_filters->to_template();
}
...
$sql = 'SELECT COUNT(t.topic_id) AS num_topics 
	...
	WHERE t.forum_id = ' . $forum_id . '
		AND ((t.topic_type <> ' . POST_GLOBAL . '
				' . $sql_limit_time . 
					$topic_filters->get_sql() . // filters mod				
				')
...
// archive mod end // topic title prefix mod end
...
// archive mod begin
	...
	'S_FORUM_IS_ARCHIVE'	=> $archive_forum,

// archive mod end

// filters mod begin

	'S_FILTER_ACTION'		=> append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_id),
...
// link mod begin 
	...
	'S_LINK_VIEW'			=> $links_forum && $user->link_view,
	'S_LINK_VIEW_ACTION'	=> append_sid($phpbb_root_path . 'viewforum.' . $phpEx, 'f=' . $forum_id . (($start == 0) ? '' : '&amp;start=' . $start) . $topic_filters->get_u() . $u_link_view . ((strlen($u_sort_param)) ? '&amp;' . $u_sort_param : '')),
...
//	links mod begin // circle mod begin

	'S_LINKS_CIRCLES_FORUM'	=> $links_circles_forum,
	
	'TLC_TOPICS'			=> $repl->lang('TOPICS'),
	'TLC_NO_TOPICS'			=> $repl->lang('NO_TOPICS'),
	'TLC_POST_TOPIC'		=> $repl->lang('POST_TOPIC'),
	'TLC_POST_TOPIC_B'		=> $repl->lang('POST_TOPIC_B'),
	'TLC_SEARCH_FORUM'		=> $repl->lang('SEARCH_FORUM'),
	'TLC_MARK_TOPICS_READ'	=> $repl->lang('MARK_TOPICS_READ'),
	'TLC_LAST_POST'			=> $repl->lang('LAST_POST'),

// link mod end // circle mod end

// hide forum permissions mod begin

	'S_FORUM_PERMISSIONS'	=> false,

// hide forum permissions mod end
	...
	'S_FORUM_ACTION'		=> append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$forum_id" . (($start == 0) ? '' : "&amp;start=$start") . $topic_filters->get_u()),
...
$template->assign_vars(array(
	'PAGINATION'	=> generate_pagination(append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_id . $topic_filters->get_u() . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '')), $topics_count, $config['topics_per_page'], $start),		// archive mod // topic title prefix mod 
	'PAGE_NUMBER'	=> on_page($topics_count, $config['topics_per_page'], $start),
	'TOTAL_TOPICS'	=> ($s_display_active) ? false : (($total_topic_count == 1) ? $repl->lang('VIEW_FORUM_TOPIC') : sprintf($repl->lang('VIEW_FORUM_TOPICS'), $total_topic_count)))
);
...
$calendar_data = obtain_calendar_suffixes($topic_list);	

// calendar mod end

// locations mod begin

$topic_locations->set_topics($topic_list);

// locations mod end

// link mod begin

$topic_list_thumbnail = new topic_list_thumbnail();

// link mod end

// hide announce mod begin

$hide_announce = false;

if ($config['hide_announce_enable'] && sizeof($announcement_list))
{
	$hide_announce_filter = request_var('annf', '');

	switch ($hide_announce_filter)
	{
		case 'show':

			$user->hide_global = 0;
			unset($user->hide_announce[$forum_id]);
			
			$serialized_hide_announce =	(sizeof($user->hide_announce)) ? serialize($user->hide_announce) : 0;		
			
			$sql = 'UPDATE ' . USERS_TABLE . '
						SET user_hide_global = ' . $user->hide_global . ",
							user_hide_announce = '" . $db->sql_escape($serialized_hide_announce) . "'
						WHERE user_id = " . $user->data['user_id'];
			$db->sql_query($sql);
		...
		case 'hide':
			...
			$user->hide_global = time();
			$user->hide_announce[$forum_id] = time();		
		...
		default:
			...
			{
				if ($rowset[$row]['topic_type'] == POST_GLOBAL)
				{
					if ($user->hide_global < $rowset[$row]['topic_time'])
					{
						$hide_announce = false;
						break;
					}
				...
				{
					if (!isset($user->hide_announce[$forum_id]) ||				
						$user->hide_announce[$forum_id] < $rowset[$row]['topic_time'])
					{
						$hide_announce = false;
						break;					
					}	
	...
	$u_hide_announce = '&annf=';
	$u_hide_announce .= ($hide_announce) ? 'show' : 'hide';
}

$template->assign_vars(array(
	'S_HIDE_ANNOUNCE'			=> $hide_announce,
	'S_HIDE_ANNOUNCE_ENABLE'	=> $config['hide_announce_enable'] && sizeof($announcement_list),
	'S_HIDE_ANNOUNCE_ACTION'	=> append_sid($phpbb_root_path . 'viewforum.' . $phpEx, 'f=' . $forum_id . (($start == 0) ? '' : '&amp;start=' . $start) . $topic_filters->get_u() . $u_hide_announce . ((strlen($u_sort_param)) ? '&amp;' . $u_sort_param : '')),
	));
	
// hide announce mod end
...
// link mod begin
			...
			'U_LINK_URL'			=>	($row['topic_is_link']) ? $row['topic_link_url'] : '',
			...
			'LINK_FORUM_STR'		=> $topic_link->forum_str($row['topic_link_forum_id']),				
			'TOPIC_TITLE_THUMB'		=> $topic_list_thumbnail->get_img($topic_id, $row['topic_thumbnail_ext']),
		
// link mod end

// Topic Title Prefix Mod - Begin

			'TITLE_PREFIX'			=> ($prefix_names[$row['topic_prefix_id']]) ? $prefix_names[$row['topic_prefix_id']] : '',
			
// Topic Title Prefix Mod - End

// archive mod begin

			'S_ARCHIVE_UNKNOWN'		=> ($row['archive_forum_id'] == 0) ? true : false,
			'ARCHIVE_FORUM_NAME'	=> $archive_forum_list[$row['archive_forum_id']]['forum_name'],
			'ARCHIVE_PARENT_NAME'	=> $archive_forum_list[$archive_forum_list[$row['archive_forum_id']]['parent_id']]['forum_name'],
			'U_ARCHIVE_FROM_FORUM'	=> append_sid($phpbb_root_path . 'viewforum.' . $phpEx, 'f=' . $row['archive_forum_id']),			
			
// archive mod end

// topic color mod begin	

			'COLOR_BG'		=> $topic_color->obtain_color($row['topic_bg_color'], 'topic', false, (sizeof($rowset) % 2)),	
