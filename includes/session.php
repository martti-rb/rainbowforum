...
/*		mt_srand((double)microtime()*1000000);  // line added header mod.   */
...
//		$db->sql_return_on_error(false);
	...
	*/
	function check_ban($user_id = false, $user_ips = false, $user_email = false, $return = false)
	{
		...
		// stopforumspam mod begin
		// include($_SERVER['DOCUMENT_ROOT'].'/includes/ucp/stopforumspam-ban.php');
		// stopforumspam mod end 
...
// hide global - announce mod begin

	var	$hide_global = false;
	var $hide_announce = array();

// hide global - announce mod end
...
// circle mod begin
	...
	var $circle_permission = array();
	var $circle_name = array();
	var $circle_ary = array();
...
// hide mod begin
			...
			$this->hide_global = $this->data['user_hide_global'];
			$hide_announce = unserialize($this->data['user_hide_announce']);
			if ($hide_announce)
			{
				$this->hide_announce = $hide_announce;
			}
...
// filters mod begin
			...
			$this->hide_filters = $this->data['user_hide_filters'];

// filters mod end

// link mod begin

			$this->link_view = $this->data['user_link_view'];

// link mod end 

// circles mod begin

			$sql = 'SELECT uc.circle_id, uc.permission, tc.topic_title
						FROM ' . TOPICS_TABLE . ' tc
						LEFT JOIN ' . USERS_CIRCLES_TABLE . ' uc ON (uc.circle_id = tc.topic_id)
						WHERE user_id = ' . $this->data['user_id'] . '
							AND uc.circle_id <> 0
							AND uc.permission <> 0
							AND tc.topic_is_circle = 1';
			$result = $db->sql_query($sql);
			while ($row = $db->sql_fetchrow($result))
			{	  
				$this->circle_permission[$row['circle_id']] = $row['permission'];
				$this->circle_ary[] = $row['circle_id'];
				$this->circle_name[$row['circle_id']] = $row['topic_title'];
			}
			$db->sql_freeresult($result);	
...
// calendar mod	begin
		...
		if ($config['calendar_enable'])
		{
			$link_open_tag = '<a href="' . append_sid($phpbb_root_path . 'calendar.' . $phpEx, 'start_jd=' . (unixtojd($gmepoch + $zone_offset) - CALENDAR_DAYS_OFFSET)) . '">';
			$link_closing_tag = '</a>';
		}
...
// calendar mod end			
		...
		if ($date_cache[$format]['is_short'] !== false && !$forcedate && !($gmepoch < $midnight - 86400 || $gmepoch > $midnight + 172800))
		{
			...
			{
				// calendar mod repl.
				return $link_open_tag . str_replace('||', $this->lang['datetime'][$day], strtr(@gmdate($date_cache[$format]['format_short'], $gmepoch + $zone_offset), $date_cache[$format]['lang'])) . $link_closing_tag;
			}
		}
		// calendar mod repl.
		return $link_open_tag . strtr(@gmdate($date_cache[$format]['format_long'], $gmepoch + $zone_offset), $date_cache[$format]['lang']) . $link_closing_tag;
	}
