...
// forum color mod begin				
				...
				'FORUM_BG_COLOR'		=> $forum_color->obtain_color($row['forum_bg_color'], 'forum'),
...
// 	link mod begin
		...
		if ($forum_id == $config['link_forum_id'])
		{
			$folder_image = 'link_forum';
		}	
...
// circle mod begin
		...
		if ($forum_id == $config['circle_forum_id'])
		{
			$folder_image = 'circle_forum';
		}
...
// Display Parent Forum Name Mod - Begin
		$template->assign_vars(array(
			...
			'PARENT_FORUM_NAME'			=> $parent_name,
			'U_VIEW_PARENT_FORUM'		=> append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $parent_forum_id)
		));
...
{
	global $user, $config, $circles; // circle mod ($circles)
...
// link mod end
// circle mod begin
	if ($topic_row['topic_is_circle'])
	{
		$folder_img = 'circle';
		$folder_img .= ($user->circle_access[$topic_row['topic_circle_id']]) ? '' : '_locked';		
		$folder_img .= ($circles->hidden[$topic_row['topic_circle_id']]) ? '_hidden' : '';
	}
	else
//
	{
