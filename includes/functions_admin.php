...
// Display Parent Forum Mod - Begin - replace		
		$sql = 'SELECT forum_id, forum_name, parent_id, forum_type, left_id, right_id, forum_parents, display_parent_forum
			FROM ' . FORUMS_TABLE . '
			ORDER BY left_id ASC';
...
// Display Parent Forum Name Mod - Begin				
			...
			$row['forum_name'] = obtain_parent_forum_name($row) . $row['forum_name'];
...
// Calendar Mod - Begin - replace
	$table_ary = array(BOOKMARKS_TABLE, TOPICS_TRACK_TABLE, TOPICS_POSTED_TABLE, POLL_VOTES_TABLE, POLL_OPTIONS_TABLE, TOPICS_WATCH_TABLE, TOPICS_TABLE, CALENDAR_PERIODS_TABLE);
// Calendar Mod - End - replace
...
// link mod replace begin
	
	$table_ary = array(POSTS_TABLE, REPORTS_TABLE, POSTS_LINKS_TABLE);

// link mod replace end
...
/*		$update_count_link_topics = array_intersect(array_unique($remove_ids), array_unique($topic_ids));

		if (sizeof($update_count_link_topics))
		{
			$sql = 'SELECT post_link_count, 
				FROM ' . POSTS_TABLE . '
				WHERE ' . $db->sql_in_set('topic_id', $update_count_link_topics) . '
				GROUP BY topic_id';
			$result = $db->sql_query($sql);
			...
			$sql = 'UPDATE ' . TOPICS_TABLE . '
				SET topic_link_count = ' . $substract . '
				WHERE ' . $db->sql_in_set('topic_id', $update_count_link_topics);
			$db->sql_query($sql);
