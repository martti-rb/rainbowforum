...
// limit days mod begin
		
		$sort_ary['output'] .= ' onchange="document.forms[\'filterform\'].submit();"';  
...
// limit days mod end		
		...
		{
			$sort_ary['output'] .= '<option value="' . $option . '"' . (($selected == $option) ? ' selected="selected"' : '');

// limit days mod begin			
	
			$sort_ary['output'] .= ($key == 'sort_days' && (substr($text,-1)) == '-') ? ' disabled="disabled"' : '';
	
// limit days mod end
			
			$sort_ary['output'] .= '>' . $text . '</option>';
		}
...
function parse_attachments($forum_id, &$message, &$attachments, &$update_count, $preview = false)
{
	...
	foreach ($attachments as $attachment)
	{
		...
		if (!$denied)
		{
			...
			{
				if ($attachment['thumbnail'] && !$links_circles_forum)
				{
...
// link mod // circle mod begin		
			...
			if ($forum_id = $config['link_forum_id'] || $forum_id = $config['circle_forum_id'])
			{
				$display_cat = ATTACHMENT_CATEGORY_NONE;
			}
...
* The default max length is 60 at the moment : EDITED TO 100
...
*/
function truncate_string($string, $max_length = 100, $max_store_length = 255, $allow_reply = false, $append = '')
{
	$chars = array();
