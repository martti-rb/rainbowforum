...
// region mod begin
/*
CREATE TABLE IF NOT EXISTS `phpbb_continents` (
  `code` CHAR(2) NOT NULL COMMENT 'Continent code',
  `name` VARCHAR(255),
  PRIMARY KEY (`code`)
);

INSERT INTO `phpbb_continents` VALUES
  ('AF', 'Africa'),
  ('AS', 'Asia'),
  ('EU', 'Europe'),
  ('NA', 'North America'),
...
CREATE TABLE IF NOT EXISTS `phpbb_countries` (
...
INSERT INTO `phpbb_countries` (`code`, `continent_code`, `name`, `iso3`, `number`, `full_name`) VALUES
...
if (isset($continents))
{
	...
	{
		$sql = 'INSERT INTO ' . REGIONS_TABLE . ' ' . $db->sql_build_array('INSERT', array(
			'code'		=> $continent['code'],
			'name'		=> $continent['name'],
			'parent_id'	=> 0,
			'layer'		=> 1)
...
$sql = 'SELECT c.code, c.name, r.id  
			FROM ' . REGIONS_TABLE . ' r
			LEFT JOIN phpbb_countries c ON (c.continent_code = r.code)
			WHERE 1 = 1';
$result = $db->sql_query($sql);
...
if (isset($countries))
{
	...
	{
		$sql = 'INSERT INTO ' . REGIONS_TABLE . ' ' . $db->sql_build_array('INSERT', array(
			'code'		=> (int) $countrie['code'],
			'name'		=> (int) $countrie['name'],
			'parent_id'	=> $countrie['id'],
			'layer'		=> 2)
		);
		$db->sql_query($sql);	
