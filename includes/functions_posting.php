...
// link mod begin
	...
	$sql = 'SELECT pl.post_id, pl.link_id, t.forum_id,
			...
			FROM ' . TOPICS_TABLE . ' t
			LEFT JOIN ' . POSTS_LINKS_TABLE . ' pl ON (pl.link_id = t.topic_id)
			WHERE ' . $db->sql_in_set('pl.post_id', $post_list) . '
				AND t.topic_is_link = 1';

	$result = $db->sql_query($sql);
	while($row = $db->sql_fetchrow($result))
	{
		$links[$row['post_id']][] = $row;
	}
...
// link mod begin		
			...
			'S_HAS_LINKS'			=> (sizeof($links)) ? true : false,
...
// link mod begin		
	...
	if (!empty($links[$row['post_id']]))
	{
		foreach ($links[$row['post_id']] as $link)
		{
			$template->assign_block_vars($mode . '_row.link', array(
				'LINK_FOLDER_IMG'		=> $user->img('link'),
				'LINK_TITLE'			=> $link['topic_title'],
				'LINK_TITLE_PREFIX'		=> ($link['topic_prefix_id']) ? $prefixes[$link['topic_prefix_id']] : '',
				'U_LINK'				=> append_sid($phpbb_root_path . 'viewtopic.' . $phpEx, 't=' . $link['link_id']. '&amp;f=' . $link['forum_id']),
				'U_LINK_URL'			=> $link['topic_link_url'],
				'LINK_STATUS_TEXT'		=> $topic_link->status[$link['topic_link_status_id']],
				'LINK_STATUS_CLASS'		=> $topic_link->status_class[$link['topic_link_status_id']],
				'LINK_TYPE_SOURCE_STR'	=> $topic_link->type_source_str($link['topic_link_type_id'], $link['topic_link_source_id']),
				'LINK_BG_COLOR'			=> $topic_color->obtain_color($link['topic_bg_color'], 'topic', $topic_data['topic_bg_color_dark']),
			));
		}
	}	
...
// topic color mod begin
				...
				'topic_bg_color'			=> $data['topic_bg_color'],
				'topic_bg_color_dark'		=> $data['topic_bg_color_dark'],
				'topic_bg_color_en'			=> 1,
...
// topic title prefix mod begin
				...
				'topic_prefix_id'			=> $data['topic_prefix_id'],

// topic title prefix mod end

// calendar mod begin

 				'calendar_period_num'		=> sizeof($data['calendar']),
				'calendar_min_year'			=> get_calendar_min_year($data['calendar']),
				'calendar_max_year'			=> get_calendar_max_year($data['calendar']),

// calendar mod end

// locations mod begin

				'location_num'				=> ($data['locations']) ? sizeof($data['locations']) : 0,
				'location_a1'				=> ($data['locations'][0]['location_a']) ? $data['locations'][0]['location_a'] : 0,
				'location_a2'				=> ($data['locations'][1]['location_a']) ? $data['locations'][1]['location_a'] : 0,
				'location_b1'				=> ($data['locations'][0]['location_b']) ? $data['locations'][0]['location_b'] : 0,
				'location_c1'				=> ($data['locations'][0]['location_c']) ? $data['locations'][0]['location_c'] : 0,
				
// locations mod end

// link mod begin
				
				'topic_link_url'			=> $data['topic_link_url'],
				'topic_is_link'				=> ($data['topic_link_url']) ? 1 : 0,
				'topic_link_status_id'		=> $data['topic_link_status_id'],				
				'topic_link_type_id'		=> $data['topic_link_type_id'],
				'topic_link_source_id'		=> $data['topic_link_source_id'],
				'topic_link_forum_id'		=> $data['topic_link_forum_id'],
...
// calendar mod begin
	...
	switch ($post_mode)
	{
		...
		case 'edit_topic':
			...
			$sql = 'DELETE FROM ' . CALENDAR_PERIODS_TABLE . '
				WHERE topic_id = ' . $data['topic_id'];
			$db->sql_query($sql);	
		...
		case 'post':
			...
			post_calendar_periods($data['topic_id'], $data['calendar']);
...
// locations mod begin
	...
	switch ($post_mode)
	{
		...
		case 'post':
			...
			post_locations($data['topic_id'], $data['locations']);
...
// link mod begin
	...
	switch ($post_mode)
	{
		...
		case 'edit':
			...
			$sql = 'DELETE FROM ' . POSTS_LINKS_TABLE . '
				WHERE post_id = ' . $data['post_id'];
			$db->sql_query($sql);	
		...
		case 'reply':
			...
			post_links($data['post_id'], $data['topic_id'], $data['link_ids']);

		break;
	}
	
	
	if ($data['forum_id'] == $config['link_forum_id'] || $data['forum_id'] == $config['circle_forum_id'])
	{
		$topic_thumbnail = new topic_thumbnail($data['topic_id']);
		...
		switch ($post_mode)
		{
			...
			case 'edit_topic':
				...
				$topic_thumbnail->clear();
		
			// no break
			
			case 'post':

				$topic_thumbnail->create();
...
function post_calendar_periods($topic_id, $calendar_data = array())
{
	global $db;
	
	if (sizeof($calendar_data))
	{
		...
		$db->sql_multi_insert(CALENDAR_PERIODS_TABLE, $sql_insert_ary);
		unset($sql_insert_ary);	
	}	
}

function get_calendar_min_year($calendar_data = array())
{
	...
	foreach ($calendar_data as $data)
	{
		$greg = cal_from_jd($data['start_jd'], CAL_GREGORIAN);
		$min = ($min) ? (($min > $greg['year']) ? $greg['year'] : $min) : $greg['year'];
	}

	return $min;
}

function get_calendar_max_year($calendar_data = array())
{
	if (!is_array($calendar_data))
	{
		return 0;
	...
	foreach ($calendar_data as $data)
	{
		$greg = cal_from_jd($data['end_jd'], CAL_GREGORIAN);
		$max = ($max) ? (($max < $greg['year']) ? $greg['year'] : $max) : $greg['year'];
	}
...
function get_calendar_post($cal_month, $cal_day, $cal_year, &$post_data, &$error)
{
	global $config, $user;
	
	if (sizeof($cal_month))
	{
		$max_periods = $post_data['calendar_max_periods'];
	
		foreach ($cal_month as $key => $month)
		{
			$s_start_jd = ($key % 2) ? false : true;
			$cal_key = floor($key / 2);
			
			if ($cal_key >= $max_periods)
			{
				$error[] = sprintf($user->lang['CALENDAR_MAX_PERIODS_EXCEDED'], $max_periods);
				break;
			}
			...
			if (!checkdate($month, $day, $year))
			{
				if (!$s_start_jd)
				{
					$calendar_data[$cal_key]['end_jd'] = $calendar_data[$cal_key]['start_jd'];
				}
			...
			if ($key == 0)
			{
				$calendar_data[0]['start_jd'] = $jd;
				continue;
			}
		
			if ($s_start_jd)
			{
				$prev_end_jd = $calendar_data[$cal_key - 1]['end_jd'];
				$min_gap = $post_data['calendar_min_gap'];
				$max_gap = $post_data['calendar_max_gap'];
				
				if ($jd <= $prev_end_jd)
				{
					$error[] = $user->lang['CALENDAR_DATES_WRONG_ORDER'];
					break;
				}
				
				if ($jd <= ($prev_end_jd + $min_gap))
				{
					$error[] = sprintf($user->lang['CALENDAR_MIN_DAYS_BETWEEN'], $min_gap);
					break;
				}

				if ($jd > ($prev_end_jd + $max_gap + 1))
				{
					$error[] = sprintf($user->lang['CALENDAR_MAX_DAYS_BETWEEN'], $max_gap);
					break;
				}
				
				$calendar_data[$cal_key]['start_jd'] = $jd;
			}
			else
			{
				$start_jd = $calendar_data[$cal_key]['start_jd'];
				$max_length = $post_data['calendar_max_period_length'];
				...
				if ($jd >= ($start_jd + $max_length))
				{
					$error[] = sprintf($user->lang['CALENDAR_TOO_LONG_PERIOD'], $max_length);
					break;
				}
			
				$calendar_data[$cal_key]['end_jd'] = $jd;
			}
		}
	}
	
	$post_data['calendar'] = $calendar_data;
}
...
function post_locations($topic_id, $locations_data = array())
{
	global $db;
	
	if (sizeof($locations_data) && $locations_data[0]['location_a'])
	{
		...
		$db->sql_multi_insert(TOPICS_LOCATIONS_TABLE, $sql_insert_ary);
		unset($sql_insert_ary);	
	}	
}
...
function post_links($post_id, $topic_id, $link_ids = array())
{
	global $db;
	...
	if (sizeof($link_ids))
	{
		...
		$db->sql_multi_insert(POSTS_LINKS_TABLE, $sql_insert_ary);
		unset($sql_insert_ary);	
	}
	
	$sql = 'UPDATE ' . POSTS_TABLE . '
		SET post_link_count = ' . sizeof($link_ids) . '
		WHERE post_id = ' . $post_id;
	$db->sql_query($sql);

	update_topic_link_count($topic_id);
}

/**/

function update_topic_link_count($topic_ids)
{
	global $db;
	...
	$topic_ids = (is_array($topic_ids)) ? array_unique($topic_ids) : array((int) $topic_ids);

	foreach ($topic_ids as $topic_id)
	{
		$topic_link_count = 0;
		
		$sql = 'SELECT post_link_count
			FROM ' . POSTS_TABLE . '
			WHERE topic_id = ' . $topic_id;
		$result = $db->sql_query($sql);	
		while($row = $db->sql_fetchrow($result))
		{
			$topic_link_count += $row['post_link_count'];
		}
		$db->sql_freeresult($result);	

		$sql = 'UPDATE ' . TOPICS_TABLE . '
			SET topic_link_count = ' . $topic_link_count . '
			WHERE topic_id = ' . $topic_id;
		$db->sql_query($sql);
	}
}
