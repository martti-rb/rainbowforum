			...
			case 'unapproved_posts':
				...
				if ($mode == 'unapproved_posts')
				{
					...
					if (sizeof($post_ids))
					{
						...
						while ($row = $db->sql_fetchrow($result))
						{
// Calendar Mod - Begin // Topic Title Prefix Mod - Begin
							$topic_list[] = $row['topic_id'];
// Calendar Mod - End // Topic Title Prefix Mod - End						
...
// Calendar Mod - Begin // Topic Title Prefix Mod - Begin	// location mod begin					
						$topic_list[] = $row['topic_id'];					
// Calendar Mod - End // Topic Title Prefix Mod - End	// location mod end	
				
					}
					$db->sql_freeresult($result);
				}

// Display Parent Forum Mod Begin - replace
				if (sizeof($forum_names))
				{
					// Select the names for the forum_ids
					$sql = 'SELECT f2.forum_id, f2.forum_name, f2.display_parent_forum, f1.forum_name AS parent_name
						FROM ' . FORUMS_TABLE . ' f1
						LEFT JOIN ' . FORUMS_TABLE . ' f2 ON (f2.parent_id = f1.forum_id)
						WHERE ' . $db->sql_in_set('f2.forum_id', $forum_names);
					$result = $db->sql_query($sql, 3600);

					$forum_names = array();
					while ($row = $db->sql_fetchrow($result))
					{
						$parent_forum_name = ($row['display_parent_forum']) ? $row['parent_name'] . ' <strong>&#8249;</strong> ' : '';
						$forum_names[$row['forum_id']] = $parent_forum_name . $row['forum_name'];
					}
					$db->sql_freeresult($result);
				}
// Display Parent Forum Mod End - replace
...
// Calendar Mod - Begin		
						'DATE_SUFFIX'	=> (isset($calendar_data[$row['topic_id']]['suffix'])) ? $calendar_data[$row['topic_id']]['suffix'] : '',      					
// Calendar Mod - End

// Topic Title Prefix Mod - Begin
						'TITLE_PREFIX'	=> (isset($prefix_data[$row['topic_id']])) ? $prefix_data[$row['topic_id']] : '',
// Topic Title Prefix Mod - End	

// locations mod begin
						'LOCATION_TAG_STRING'	=> 	$topic_locations->get_tag_string($row['topic_id']),
// locations mod end
