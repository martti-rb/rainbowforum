	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'overview':
				...
				$report_sfs		= request_var('report_sfs', 0);
...
// sfs mod - Begin		
							$bs_sfs = unserialize($config['bs_sfs']);
								...
								if ($bs_sfs['sfs']['api_key'] && $report_sfs)
								{
									$sfs_url = 'http://www.stopforumspam/add';
									$sfs_fields = 'username=' . $user_row['username'] . '&';
									$sfs_fields .= 'ip=' . $user_row['user_ip'] . '&';
									$sfs_fields .= 'email=' . $user_row['user_email'] . '&';
									$sfs_fields .= 'api_key=' . $bs_sfs['sfs']['api_key'];
									
									$fp = fsockopen("www.stopforumspam.com",80);
									fputs($fp, "POST /add.php HTTP/1.1\n" );
									fputs($fp, "Host: www.stopforumspam.com\n" );
									fputs($fp, "Content-type: application/x-www-form-urlencoded\n" );
									fputs($fp, "Content-length: ".strlen($sfs_fields)."\n" );
									fputs($fp, "Connection: close\n\n" );
									fputs($fp, $sfs_fields);
									fclose($fp);								
									
									add_log('admin', 'LOG_USER_REPORT_SFS', $sfs_fields);
...
// calendar mod begin				
							...
							'user_bday_day'		=> $data['bday_day'],
							'user_bday_month'	=> $data['bday_month'],
							'user_bday_year'	=> $data['bday_year'],
