...
$topic_mod .= ($auth->acl_get('m_move', $forum_id) && ($topic_data['topic_status'] != ITEM_MOVED) && $config['archive_id']) ? '<option value="move_to_archive">' . $user->lang['MOVE_TOPIC_TO_ARCHIVE'] . '</option>' : '';
...
$topic_thumbnail->set_extension($topic_data['topic_thumbnail_ext']);

// link mod end // circle mod end

// link mod begin

$topic_link = new topic_link(true, false);

// link mod end 

// archive mod begin 

if ($config['archive_id'] == $forum_id)
	{ 
	if ($topic_data['archive_forum_id'] == 0)
		{
			$archive_data['archive_forum_name'] = '';
			$archive_data['archive_parent_name']	= '';		
			$archive_data['unknown'] = true; 	
		}
		else
		{
			$sql = 'SELECT 	f1.forum_name as archive_parent_name, 
							f2.forum_name as archive_forum_name					
					FROM ' . FORUMS_TABLE . ' f1
						LEFT JOIN ' . FORUMS_TABLE . ' f2 ON (f1.forum_id = f2.parent_id)
					WHERE f2.forum_id = ' . $topic_data['archive_forum_id'];
			$result = $db->sql_query($sql);	
			$archive_data = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);
			
			$archive_data['unknown'] = false; 
		}
	}
...
if ($config['calendar_multi_suffix'])
{
	$calendar_data = obtain_calendar_suffixes($topic_id, NULL, true);	
	
	if (sizeof($calendar_data[$topic_id]['multi']) > 1)
	{
		$calendar_multi = true;
		
		foreach ($calendar_data[$topic_id]['multi'] as $cal_period)
		{
			$template->assign_block_vars('calendar_multi', array(
				'DATE_SUFFIX'		=> $cal_period['suffix'],
				'U_CALENDAR_TOPIC'	=> append_sid($phpbb_root_path . 'calendar.' . $phpEx, 'start_jd=' . ($cal_period['start_jd'] - CALENDAR_DAYS_OFFSET) . '&amp;t=' . $topic_id)
			));
		}
	}

	$calendar_suffix = $calendar_data[$topic_id]['single']['suffix'];
	$calendar_start_jd = $calendar_data[$topic_id]['single']['start_jd'];		
}
else
{
	$calendar_data = obtain_calendar_suffixes($topic_id);
	$calendar_suffix = $calendar_data[$topic_id]['suffix'];
	$calendar_start_jd = $calendar_data[$topic_id]['start_jd'];
}
...
// topic color mod begin
	...
	$topic_color->topic_template($topic_data['topic_bg_color'], $topic_data['topic_bg_color_dark']);
...
// Calendar Mod - Begin
	...
	'DATE_SUFFIX'			=> (isset($calendar_suffix)) ? $calendar_suffix : '',   
	'U_CALENDAR_TOPIC' 		=> (isset($calendar_start_jd)) ? append_sid($phpbb_root_path . 'calendar.' . $phpEx, 'start_jd=' . ($calendar_start_jd - CALENDAR_DAYS_OFFSET) . '&amp;t=' . $topic_id) : '',
...
// link mod begin
	...
	'U_LINK_URL'			=> $topic_data['topic_link_url'],
	'LINK_STATUS_TEXT'		=> $topic_link->status[$topic_data['topic_link_status_id']],
	'LINK_STATUS_CLASS'		=> $topic_link->status_class[$topic_data['topic_link_status_id']],
	'LINK_TYPE_SOURCE_STR'	=> $topic_link->type_source_str($topic_data['topic_link_type_id'], $topic_data['topic_link_source_id']),
	'LINK_FORUM_STR'		=> $topic_link->forum_str($topic_data['topic_link_forum_id']),
...
// link mod begin // circle mod begin
	...
	'TOPIC_TITLE_THUMB'		=> ($links_circles_forum && $topic_data['topic_thumbnail_ext']) ? $topic_thumbnail->get_img() : false,
...
// archive mod start 
	...
	'S_FORUM_IS_ARCHIVE'	=> ($config['archive_id'] == $forum_id) ? true : false,
	'S_ARCHIVE_UNKNOWN'		=> $archive_data['unknown'],
	'ARCHIVE_FORUM_NAME'	=> $archive_data['archive_forum_name'],
	'ARCHIVE_PARENT_NAME'	=> $archive_data['archive_parent_name'],
	'U_ARCHIVE_FROM_FORUM'	=> append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $topic_data['archive_forum_id']),
	'U_ARCHIVE_FILTER'		=> append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_id . '&amp;archf=' . ($topic_data['archive_forum_id'] ? $topic_data['archive_forum_id'] : 'unknown')),
...
// topic title prefix mod begin
	...
	'TITLE_PREFIX'			=> ($topic_data['topic_prefix_id']) ? $prefixes[$topic_data['topic_prefix_id']] : '',
...
// link mod begin
		...
		'S_HAS_LINKS'			=> ($links[$row['post_id']]) ? true : false,
...
// link mod end 	
		...
		'U_QUOTE'			=> ($auth->acl_get('f_reply', $forum_id) && !$links_circles_forum) ? append_sid("{$phpbb_root_path}posting.$phpEx", "mode=quote&amp;f=$forum_id&amp;p={$row['post_id']}") : '',
