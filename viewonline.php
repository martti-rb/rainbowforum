...
// Forum info
$sql = 'SELECT forum_id, forum_name, parent_id, forum_type, left_id, right_id, display_parent_forum
	FROM ' . FORUMS_TABLE . '
	ORDER BY left_id ASC'; // Display Forum Parent Mod
...
// Display Forum Parent Name Mod - Begin

				$parent_forum_name = $forum_data[$forum_data[$forum_id]['parent_id']]['forum_name'];
				$parent_forum_name = ($forum_data[$forum_id]['display_parent_forum'] && isset($parent_forum_name)) ? $parent_forum_name . ' <strong>&#8249;</strong> ' : '';
				$forum_name = $parent_forum_name . $forum_data[$forum_id]['forum_name'];
				
// Display Forum Parent Name Mod - End				

				if ($forum_data[$forum_id]['forum_type'] == FORUM_LINK)
				{
					$location = sprintf($user->lang['READING_LINK'], $forum_name); // Display Forum Parent Name Mod
					break;
				}
				...
				switch ($on_page[1])
				{
					case 'posting':
						...
						switch ($posting_mode)
						{
							case 'reply':
							case 'quote':
								$location = sprintf($user->lang['REPLYING_MESSAGE'], $forum_name); // Display Forum Name Mod 
							break;

							default:
								$location = sprintf($user->lang['POSTING_MESSAGE'], $forum_name); // Display Forum Name Mod 
							break;
						}
					break;

					case 'viewtopic':
						$location = sprintf($user->lang['READING_TOPIC'], $forum_name); // Display Forum Name Mod 
					break;

					case 'viewforum':
						$location = sprintf($user->lang['READING_FORUM'], $forum_name); // Display Forum Name Mod 
					break;
				}
			}
			else
...
//map mod begin
		...
		case 'map':        
			$location = $user->lang['VIEWING_MAP'];
			$location_url = append_sid("{$phpbb_root_path}map.$phpEx");
		break;	
